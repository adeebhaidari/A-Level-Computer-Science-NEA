...
# A* solver
def astar_white_cross_solver(start_cube, max_depth=20, time_limit=30.0):
    start_state = start_cube.get_state_id()
    heap = []
    heappush(heap, (white_cross_heuristic(start_cube), 0, start_state, []))
...
if __name__ == "__main__":
    ...
    print("Searching for white cross (A* with pruning)...")
    sol = astar_white_cross_solver(c, max_depth=20, time_limit=30.0)
    if sol:
